...
{
    for (auto i : step(3)){
        seme_soubi << Texture{U"bns-gamejam/images/{}/seme_soubi_{}.png"_fmt(!(getData().stage_id) ? U"nabe" : U"pafe", i+1)};
        nige_soubi << Texture{U"bns-gamejam/images/{}/nige_soubi_{}.png"_fmt(!(getData().stage_id) ? U"nabe" : U"pafe", i+1)};
    }
    bgm.play(1s);
    bgm.setVolume(0.5);
}
